...
controlled migration slices applied after that audit. The target is the Grok
CLI checkout; `ShelraCode/` is a nested checkout used as the local reference.
...
|---|---|---|---|
| Target (`superagent-ai/grok-cli` derived) | `D:\\PROYECTS\\grok-cli` | `main`, `fb97af83f06dca873281d60168430f06c8de6324` (`bump version`) | migration changes are intentionally uncommitted; nested `ShelraCode/` reference checkout is ignored |
| Reference (`yosoyjavieruiz/ShelraCode`) | `D:\\PROYECTS\\grok-cli\\ShelraCode` | `main`, `ce4890e6d2def1ed50bd4ced197e6e2f2dad59c9` | clean |
...
Target instructions are in `AGENTS.md`: Bun commands, the broken ESLint 9/legacy config, the type-only import runtime issue in `src/utils/model-config.ts`, and the current `GROK_API_KEY` requirement. Target has `.agents/skills/{agent-browser,agent-desktop,find-skills}`.
...
the nested reference checkout. It retains the Grok compatibility dependency
behind an adapter, plus a direct OpenAI-compatible SDK for local runtimes.
...
1. **Where does Grok UI depend on xAI?** `src/ui/app.tsx` imports the Grok model registry, displays Grok/xAI credential text, validates `xai-` keys, renders “Message Grok…”, and exposes Grok update/Telegram paths. The UI also consumes target-specific `Agent`, `ToolResult`, `ToolCall`, and model types. It does not call the SDK directly, but it is coupled to provider-owned vocabulary and state.
2. **Smallest replacement boundary:** the call made by `src/agent/agent.ts` around `streamText` (currently `src/agent/agent.ts:1824-1980`) plus title/recap generation in `src/grok/client.ts:43-117`. The boundary must own normalized model metadata, request/stream events, tool calls/results, usage, cancellation, and typed failures. Tool execution and UI events must remain outside provider adapters.
3. **Evolve or replace Grok loop?** Evolve the Grok shell and session-facing `Agent` façade, while adapting Shelra's live `src/agent/loop.ts:936-6030` as the future execution kernel. Do not port Shelra's TUI and do not keep two long-lived agent loops.
4. **Stronger Grok tools:** mature terminal UX, MCP/LSP presentation, session resume, command palette/slash flows, media/computer views, and existing stream-to-UI observer events (`src/agent/agent.ts:114-158`).
...
7. **Features fundamentally xAI-dependent:** `@ai-sdk/xai` model creation, Grok model/pricing registry, X/web search tools, xAI batch HTTP, Grok Imagine media, xAI STT, x402 wallet/payment integration, and API-key-only startup. Each needs a capability decision before removal.
...
13. **Capability measurement:** Shelra has runtime discovery plus optional executable capability probes and cached `ModelCapabilityEvidence` in `src/cli/control-plane.ts:311-508`; target currently trusts static Grok metadata.
...
20. **Settings migration:** read old `~/.grok`/`.grok` settings in a one-way compatibility reader, validate, write only Shelra settings/state, and retain an explicit migration marker. Do not rename paths in Phase 0.
...
22. **Temporary compatibility:** a Grok adapter behind the normalized provider contract, a target `Agent` façade over Shelra task execution, legacy settings reader, and feature capability flags. Each has a deletion phase in `07-MIGRATION-PLAN.md`.
23. **Deletion timing:** adapter after Phase 6 local execution proof; façade/bridges after Phase 7 reconciliation; Grok names and paths in Phase 8; all dead code in Phase 9.
...
path does not select xAI or an xAI endpoint. The legacy xAI adapter, Grok model
catalog, STT/media/search/batch modules, and mature `.grok` artifact paths are
isolated compatibility surfaces recorded for the deliberate Phase 9 removal
decision.
