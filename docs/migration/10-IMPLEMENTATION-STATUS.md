...
the cloud-first runtime implementation. The Grok-derived React/OpenTUI shell
...
| --- | --- | --- |
| 0 - forensic audit | IMPLEMENTED | `docs/migration/00-CURRENT-STATE.md` through `09-RENAME-MATRIX.md` |
...
| 6 - remove xAI execution | ADAPTED | default startup never creates the xAI client; legacy xAI modules remain only for explicit compatibility callers and require a deliberate breaking-release cleanup |
| 7 - feature reconciliation | IMPLEMENTED, capability dependent | Grok shell commands, MCP, LSP, headless, schedules and approvals remain; unsupported provider-only capabilities surface as unavailable |
| 8 - branding/config | PARTIALLY IMPLEMENTED | new writes use `.shelra` and `SHELRA_*`; legacy `.grok` reads remain for migration compatibility |
| 9 - debt deletion | DEFERRED COMPATIBILITY DEBT | xAI/media/STT/batch compatibility modules and historical Grok paths remain isolated pending a breaking release |
...
for `--autonomous` runs. See `docs/migration/14-AGENT-HARNESS-RECONSTRUCTION.md` for the full
...
not a Grok model registry. A future catalog refresh can add more HF entries
