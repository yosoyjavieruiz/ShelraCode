...
The target has one provider path. `src/grok/client.ts:43-59` creates an xAI SDK provider from an API key/base URL and resolves a Grok chat or responses model. `src/grok/models.ts:3-111` statically describes Grok IDs, context windows, prices, reasoning effort and response/client-tool capabilities. `src/agent/agent.ts:1871-1952` selects that runtime and sends AI SDK `streamText`; title/recap use `generateText` in the same client. There is no hardware-aware eligibility or empirical capability probe in the target path.

Target model-facing features include xAI web/X search, Grok Imagine image/video, xAI batch HTTP (`src/grok/batch.ts`, `src/agent/agent.ts:956-1150`), xAI STT and x402 payment/wallet tools. They must be represented as capabilities, not silently retained after xAI removal.
