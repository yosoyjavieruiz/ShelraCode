...
Checkpoints snapshot each ShelraCode-owned path before its first mutation. Rollback refuses to overwrite a file changed after the checkpoint and never uses `git reset --hard`, `git clean`, or broad checkout.
