...
|---|---|---|---|---|
| React/OpenTUI shell | `src/ui/app.tsx`, `src/index.ts:59-106` | Shelra has a separate Solid/OpenTUI TUI | KEEP TARGET IMPLEMENTATION | Product constraint; target interaction is the foundation. |
...
| Composer/keyboard/resize | `src/ui/app.tsx:3822-4002` | Shelra TUI is unrelated | KEEP TARGET IMPLEMENTATION | Preserve focus, queue, slash, scroll and responsiveness. |
...
| Image/video generation | target xAI image/video media tools | no local media runtime evidence | REMOVE OR DEFER | Remove only after UI capability state exists; no fake backend. |
| Web/X search | xAI response tools in `src/toolset/tools.ts:59-84` | no equivalent required for local coding | REMOVE XAI / ADAPT GENERIC | X search is xAI-specific; generic search is a separate optional capability. |
| xAI Batch | `src/toolset/batch.ts`, Agent batch path | no generic batch contract | REMOVE XAI / DEFER GENERIC | Do not preserve an xAI-only flag after provider removal. |
...
| Install/update/uninstall | target install manager and release assets | Shelra installation tests/scripts | MERGE BEST OF BOTH | Preserve mature lifecycle only after license review. |
| Pricing/quota | target static xAI model prices and xAI batch economics | Shelra quota/privacy/cost policy | REPLACE WITH SHELRA | Model metadata must be provider-neutral; no paid escalation by default. |
