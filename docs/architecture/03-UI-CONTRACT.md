# Terminal UI compatibility contract
...
`App` currently depends on target `Agent`, target message/tool types, a static model registry and xAI-specific credential/copy. Provider-neutral seams should be introduced at the `Agent`/application boundary. The UI should receive a normalized model summary and presentation events; it must never receive an SDK model, xAI response object, provider-specific tool schema, or raw provider error.

Direct coupling observed in `src/ui/app.tsx` includes static model-registry imports; API key presence and `xai-` validation; “Paste your xAI API key…” copy; and update/Telegram paths using provider-specific directories. These are recorded migration targets and intentionally unchanged in Phase 0.
