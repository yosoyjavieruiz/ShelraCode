...
import { inferMimeTypeFromFileName, RemoteSttEngine } from "./remote-stt";

describe("RemoteSttEngine", () => {
  let tempDir: string;
  const originalFetch = global.fetch;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "remote-stt-test-"));
  });
  ...
  it("requires an API key", () => {
    expect(() => new RemoteSttEngine({ apiKey: "" })).toThrow(/API key/i);
    expect(() => new RemoteSttEngine({ apiKey: "   " })).toThrow(/API key/i);
  });
  ...
  it("posts the audio file as multipart form-data to /v1/stt", async () => {
    ...
    const engine = new RemoteSttEngine({
      apiKey: "test-key",
    ...
    expect(url).toBe("https://stt.example.com/v1/stt");
  ...
  it("omits language and format fields when language is not provided", async () => {
    ...
    const engine = new RemoteSttEngine({ apiKey: "test-key", baseURL: "https://stt.example.com/v1" });
  ...
  it("surfaces non-2xx responses with status and body", async () => {
    ...
    const engine = new RemoteSttEngine({ apiKey: "test-key", baseURL: "https://stt.example.com/v1" });
    await expect(engine.transcribe({ audioPath })).rejects.toThrow(
      /Speech-to-text request failed \(400\).*invalid audio/,
    );
  });
